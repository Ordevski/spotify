...
const DisplayAlbum = ( {album} ) => {
    ...
    const {playWithId, playStatus} = useContext(PlayerContext);
    ...
    return (
        <>
            ...
            { songsData./*filter((item) => item.album === album.name).*/map((item, index) => (
                <div className="grid grid-cols-3 sm:grid-cols-[0.5fr_2fr_2fr_0.5fr] gap-2 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff26] cursor-pointer"
                    key={index} onClick={() => hoveredSongId === item.id && playStatus ? pause() : playWithId(item.id)}
